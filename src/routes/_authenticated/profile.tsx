    ...
    <div className="mx-auto max-w-2xl px-5 pt-[max(env(safe-area-inset-top),1rem)]">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="flex items-center gap-1 text-sm text-muted-foreground">
            ...
            {Number(data?.profile?.rating ?? 5).toFixed(2)} · {data?.profile?.total_rides ?? 0} corridas
          </p>
          {data?.roles?.length ? (
            <div className="mt-1 flex flex-wrap gap-1">
              {data.roles.map((r: string) => (
                <span key={r} className="rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-secondary">
                  {r}
                </span>
              ))}
            </div>
      ...
      <div className="mt-6 space-y-2">
        {data?.roles?.includes("driver") ? (
          <Row icon={<Car className="size-4" />} label="Painel do motorista" to="/driver" />
        ) : (
          <Row icon={<Car className="size-4" />} label="Tornar-se motorista" to="/driver" />
        )}
        ...
        {data?.roles?.includes("admin") && <Row icon={<Shield className="size-4" />} label="Painel administrativo" to="/admin" />}
      </div>

      <Button variant="outline" className="mt-6 h-11 w-full text-sm font-semibold" onClick={signOut}>
        <LogOut className="size-4" /> Sair da conta
      </Button>

      <p className="mt-6 text-center text-[11px] text-muted-foreground">RotaMais v1.0 · feito com cuidado</p>
    </div>
  );
}
