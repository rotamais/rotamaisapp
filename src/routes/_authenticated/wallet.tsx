    ...
    <div className="mx-auto max-w-2xl px-5 pt-[max(env(safe-area-inset-top),1rem)]">
      ...
      <div className="mt-5 rounded-3xl bg-secondary p-5 text-secondary-foreground">
        ...
        <div className="mt-4 flex gap-2">
          ...
          <Button variant="outline" className="h-10 flex-1 border-secondary-foreground/20 bg-transparent text-secondary-foreground hover:bg-secondary-foreground/10">
            Sacar
          </Button>
        </div>
      </div>
      ...
      <div className="mt-3 space-y-2">
        <PayItem icon={<CreditCard className="size-4" />} title="Visa •••• 4242" subtitle="Principal" />
        <PayItem icon={<QrCode className="size-4" />} title="PIX" subtitle="Chave: cpf@rotamais" />
        <PayItem icon={<Banknote className="size-4" />} title="Dinheiro" subtitle="Pagar ao motorista" />
...
function PayItem({ icon, title, subtitle }: { icon: React.ReactNode; title: string; subtitle: string }) {
  return (
